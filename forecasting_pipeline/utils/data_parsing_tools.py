...
def load_data_bases(cfg):
    ...
    lot_cache = Path(cfg.cache_path) / "cache_lot.p"
    endog_cache = Path(cfg.cache_path) / "cache_endog.p"
    exogs_cache = Path(cfg.cache_path) / "cache_exogs.p"

    if cfg.dir == "lot":
        if lot_cache.is_file():
        ...
        if endog_cache.is_file():
            data = pickle.load(open(endog_cache, "rb"))
            print("Load cached data")
        else:
            data, meta = load_all_locations(path=cfg.source_path + "/Datenpaket_BBD")
            pickle.dump(data, open(endog_cache, "wb"))
    if exogs_cache.is_file():
        exogs = pickle.load(open(exogs_cache, "rb"))
        print("Load cached data")
    else:
        exogs = load_all_external(locations=cfg.all_locs, path=cfg.source_path + "/Ruhrverband")
        pickle.dump(exogs, open(exogs_cache, "wb"))
    return data, exogs